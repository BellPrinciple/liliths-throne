			...
			if(isWantsToFight()) {
				if (index == 1) {
					...
					if(Main.game.getPlayer().getMoney()<DialogueFlags.MUGGER_DEMAND_2) {
						...
						return new Response.Back("Offer money ("+UtilText.formatAsMoney(DialogueFlags.MUGGER_DEMAND_2, "span")+")",
								"Offer to pay [npc.name] "+Util.intToString(DialogueFlags.MUGGER_DEMAND_2)+" flames to leave you alone.") {
							@Override
							public void effects() {
								applyPregnancyReactions();
					...
					if(!Main.game.getPlayer().hasItemType(ItemType.MUSHROOM)) {
						...
						return new Response.Back("Offer mushroom", "Offer one of your Glowing Mushrooms to [npc.name] in exchange for leaving you alone.") {
							@Override
							public void effects() {
								if(getMugger().isVisiblyPregnant()){
									getMugger().setCharacterReactedToPregnancy(Main.game.getPlayer(), true);
