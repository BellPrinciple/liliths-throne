...
public enum PerkManager {
	...
	private Map<Integer, Map<PerkCategory, List<TreeEntry<PerkCategory, Perk>>>> dollPerkTree;
	...
	private List<TreeEntry<PerkCategory, Perk>> dollStartingPerks = new ArrayList<>();
...
//		elementalStartingPerks.add(both1);
		...
		TreeEntry<PerkCategory, Perk> dollPerk = addPerkEntry(dollPerkTree, PerkCategory.PHYSICAL, 2, Perk.DOLL_PHYSICAL_1, physical1);
	...
	private static TreeEntry<PerkCategory, Perk> addPerkEntry(Map<Integer, Map<PerkCategory, List<TreeEntry<PerkCategory, Perk>>>> perkTree, PerkCategory category, int row, Perk perk, TreeEntry<PerkCategory, Perk>... links) {
		TreeEntry<PerkCategory, Perk> entry = new TreeEntry<>(category, row, perk);
	...
	public static int getInitialPerkCount(GameCharacter character, PerkCategory category) {
		int count = 0;
		for(TreeEntry<PerkCategory, Perk> entry : getStartingPerks(character)) {
	...
	public static void initialiseSpecialPerksUponCreation(GameCharacter character) {
		...
		if(!character.isUnique() && !(character.isElemental()) && rnd.nextInt(100)<=50) {
			...
			Set<Perk> specialPerks = character.getSpecialPerks();
	...
	public static Set<Perk> initialisePerks(GameCharacter character, boolean autoSelectPerks, List<Perk> requiredPerks, Map<PerkCategory, Integer> perkWeightingOverride) {
		Set<Perk> perksAdded = new HashSet<>();
		...
		for(TreeEntry<PerkCategory, Perk> perk : getStartingPerks(character)) {
		...
		if(!character.isElemental()) { // Elementals always start with an empty perk tree
			if(!character.isPlayer() && autoSelectPerks) {
				...
				if(requiredPerks!=null) {
					for(Perk requiredPerk : requiredPerks) {
						for(TreeEntry<PerkCategory, Perk> entry : Pathing.aStarPathingPerkTree(character, MANAGER.getFirstPerkEntry(character, requiredPerk))) {
				...
				List<Perk> deniedPerks = new ArrayList<>();
				...
				List<TreeEntry<PerkCategory, Perk>> traits = new ArrayList<>();
				while(character.getPerkPoints()>0) {
					PerkCategory category;
					category = Util.getRandomObjectFromWeightedMap(perkWeightingMap, rnd);
					Map<TreeEntry<PerkCategory, Perk>, Integer> weightedAvailabilityMap = new HashMap<>();
					
					for(Map<PerkCategory, List<TreeEntry<PerkCategory, Perk>>> map : MANAGER.perkTree.values()) {
						for(TreeEntry<PerkCategory, Perk> perkEntry : map.get(category)) {
					...
					if(weightedAvailabilityMap.isEmpty()) {
						...
						TreeEntry<PerkCategory, Perk> entryToAdd = Util.getRandomObjectFromWeightedMap(weightedAvailabilityMap, rnd);
				...
				for(TreeEntry<PerkCategory, Perk> trait : traits) {
	...
	public String getPerkTreeDisplay(GameCharacter character, boolean includePointsRemaining) {
		...
		if(character.getSpecialPerks().size()>0) {
			List<Perk> subspeciesKnowledgePerks = new ArrayList<>();
			...
			treeSB.append("<div class='container-full-width' style='width:100%; padding:0; margin:0;'>");
				for(Perk hiddenPerk : subspeciesKnowledgePerks) {
					treeSB.append(
							"<div id='HIDDEN_PERK_" + Perk.getIdFromPerk(hiddenPerk) + "' class='square-button round small'"
