...
public enum PerkManager {
	...
	private List<TreeEntry<PerkCategory,Perk>> elementalStartingPerks = new ArrayList<>();
	private List<TreeEntry<PerkCategory,Perk>> standardStartingPerks = new ArrayList<>();
	...
	private PerkManager() {
		...
		TreeEntry<PerkCategory,Perk> connectorLeft, connectorMid, connectorRight;
		TreeEntry<PerkCategory,Perk> leftA, leftB, leftC;
		TreeEntry<PerkCategory,Perk> leftMidA, leftMidB, leftMidC;
		TreeEntry<PerkCategory,Perk> rightMidA;
		TreeEntry<PerkCategory,Perk> rightA, rightB, rightC;
		
		TreeEntry<PerkCategory,Perk> arcane1, arcane2, arcane3, arcane4;
		TreeEntry<PerkCategory,Perk> physical1, physical2, physical3, physical4;
		TreeEntry<PerkCategory,Perk> both1, both2, both3, both4;
...
//		elementalStartingPerks.add(both1);
	...
	private static TreeEntry<PerkCategory,Perk> addPerkEntry(Map<Integer, Map<PerkCategory, List<TreeEntry<PerkCategory,Perk>>>> perkTree, PerkCategory category, int row, Perk perk, TreeEntry<PerkCategory,Perk>... links) {
		var entry = new TreeEntry<>(category, row, perk);
		perkTree.get(row).get(category).add(entry);
		
		for(var linkEntry : links) {
	...
	public boolean isPerkAnywhereInAvailableTree(Perk perk, GameCharacter character) {
		for(var entry : getPerkTree(character).values()) {
			for(var treeListEntry : entry.values()) {
				for(var treeEntry : treeListEntry) {
	...
	public static List<TreeEntry<PerkCategory,Perk>> getStartingPerks(GameCharacter character) {
		if(character.isElemental()) {
	...
	public static int getInitialPerkCount(GameCharacter character, PerkCategory category) {
		int count = 0;
		for(var entry : getStartingPerks(character)) {
	...
	public static void initialiseSpecialPerksUponCreation(GameCharacter character) {
		...
		if(!character.isUnique() && !(character.isElemental()) && rnd.nextInt(100)<=50) {
			...
			var specialPerks = character.getSpecialPerks();
			for(var perk : List.copyOf(specialPerks)) {
	...
	public static Set<Perk> initialisePerks(GameCharacter character) {
		return initialisePerks(character, true, null, null);
	}
	
	public static Set<Perk> initialisePerks(GameCharacter character, boolean autoSelectPerks) {
		return initialisePerks(character, autoSelectPerks, null, null);
	}
	
	public static Set<Perk> initialisePerks(GameCharacter character, boolean autoSelectPerks, List<Perk> requiredPerks) {
		return initialisePerks(character, autoSelectPerks, requiredPerks, null);
	}

	public static Set<Perk> initialisePerks(GameCharacter character, List<Perk> requiredPerks, Map<PerkCategory, Integer> perkWeightingOverride) {
	 ...
	 */
	public static Set<Perk> initialisePerks(GameCharacter character, boolean autoSelectPerks, List<Perk> requiredPerks, Map<PerkCategory, Integer> perkWeightingOverride) {
		var perksAdded = new HashSet<Perk>();
		
		for(var perk : getStartingPerks(character)) {
		...
		if(!character.isElemental()) { // Elementals always start with an empty perk tree
			if(!character.isPlayer() && autoSelectPerks) {
				...
				if(requiredPerks!=null) {
					for(var requiredPerk : requiredPerks) {
						for(var entry : Pathing.aStarPathingPerkTree(character, MANAGER.getFirstPerkEntry(character, requiredPerk))) {
				...
				var deniedPerks = new ArrayList<Perk>();
				...
				var traits = new ArrayList<TreeEntry<PerkCategory,Perk>>();
				while(character.getPerkPoints()>0) {
					PerkCategory category;
					category = Util.getRandomObjectFromWeightedMap(perkWeightingMap, rnd);
					var weightedAvailabilityMap = new HashMap<TreeEntry<PerkCategory,Perk>,Integer>();
					
					for(var map : MANAGER.perkTree.values()) {
						for(var perkEntry : map.get(category)) {
					...
					if(weightedAvailabilityMap.isEmpty()) {
						...
						var entryToAdd = Util.getRandomObjectFromWeightedMap(weightedAvailabilityMap, rnd);
	...
	public String getPerkTreeDisplay(GameCharacter character, boolean includePointsRemaining) {
		...
		if(character.getSpecialPerks().size()>0) {
			treeSB.append("<div class='container-full-width' style='width:100%; padding:0; margin:0;'>");
				for(var hiddenPerk : Perk.getHiddenPerks()) {
	...
	private void appendPerkColumn(GameCharacter character, int row, PerkCategory category) {
		...
		var perkList = getPerkTree(character).get(row).get(category);
		int size = perkList.size();
		
		treeSB.append(getHorizontalLine(character, perkList));
		for(var entry : perkList) {
	...
	private String getHorizontalLine(GameCharacter character, List<TreeEntry<PerkCategory,Perk>> perkList) {
		...
		for(var entry : perkList) {
			...
			for(var siblingEntry : entry.getSiblingLinks()) {
			...
			for(var parentEntry : entry.getParentLinks()) {
	...
	private float getX(GameCharacter character, int row, TreeEntry<PerkCategory,Perk> entry) {
		var list = getPerkTree(character).get(row).get(entry.getCategory());
		int size = list.size();
	...
	public boolean isPerkAvailable(GameCharacter character, TreeEntry<PerkCategory,Perk> perkEntry) {
		...
		if(!isPerkOwned(character, perkEntry)) {
			for(var linkedEntry : perkEntry.getLinks()) {
	...
	public boolean isPerkAvailable(GameCharacter character, int row, Perk perk) {
		...
		if(!isPerkOwned(character, row, perk)) {
			var o = getPerkTree(character).get(row).values().stream().flatMap(List::stream).filter(e->e.getEntry()==perk).findFirst();
			if(o.isEmpty()) {
				return false;
			}
			var perkEntry = o.get();
	 ...
	 */
	public boolean isPerkEndOfTreeBranch(GameCharacter character, int row, Perk perk, boolean onlyCheckOwnedPerks) {
		if(perk.isAlwaysAvailable()) {
			return true;
		}
		var o = getPerkTree(character).get(row).values().stream().flatMap(List::stream).filter(e->e.getEntry()==perk).findFirst();
	...
	private Colour getPerkLineParentColour(GameCharacter character, TreeEntry<PerkCategory,Perk> entry) {
		...
		for(var parent : entry.getParentLinks()) {
	...
	private Colour getPerkLineChildColour(GameCharacter character, TreeEntry<PerkCategory,Perk> entry) {
		...
		for(var child : entry.getChildLinks()) {
	...
	private Colour getPerkLineSiblingColour(GameCharacter character, TreeEntry<PerkCategory, Perk> entry) {
		...
		for(var sibling : entry.getSiblingLinks()) {
	...
	public TreeEntry<PerkCategory,Perk> getFirstPerkEntry(GameCharacter character, Perk perk) {
		var tree = getPerkTree(character);
		for(int i=0; i<ROWS; i++) {
			var o = tree.get(i).values().stream().flatMap(List::stream).filter(e->e.getEntry()==perk).findFirst();
			if(o.isPresent())
				return o.get();
		}
		return null;
	...
	public int getPerkRow(GameCharacter character, Perk perk) {
		var tree = getPerkTree(character);
		for(int i=0; i<ROWS; i++)
			if(tree.get(i).values().stream().flatMap(List::stream).map(TreeEntry::getEntry).anyMatch(perk::equals))
				return i;
