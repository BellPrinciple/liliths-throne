...
public interface BodyCoveringType {
	...
	String getNameSingular(GameCharacter character);

	String getNamePlural(GameCharacter character);

	/**
	 * @return
	 * The name of the covering for use in transformation menus.
	 * Will most likely return the same as getName(gc)
	 */
	default String getNameTransformation(GameCharacter character) {
		return getName(character);
	}

	default String getName(GameCharacter character) {
		return isDefaultPlural() ? getNamePlural(character) : getNameSingular(character);
	}

	default Map<CoveringPattern,Integer> getNaturalPatterns() {
		return Map.of(CoveringPattern.NONE,1);
	}
	...
	default List<Colour> getAllColours() {
		var result = new LinkedHashSet<>(getNaturalColoursPrimary());
		result.addAll(getDyeColoursPrimary());
		result.addAll(getNaturalColoursSecondary());
		result.addAll(getDyeColoursSecondary());
	...
	default List<Colour> getAllSecondaryColours() {
		var result = new LinkedHashSet<>(getNaturalColoursSecondary());
		result.addAll(getDyeColoursSecondary());
		return List.copyOf(result);
	}
	...
	default List<CoveringModifier> getNaturalModifiers() {
		return List.of(CoveringModifier.SMOOTH);
	}
...
//	};
	...
	@Deprecated
	public static String getIdFromBodyCoveringType(BodyCoveringType bct) {
		return bct.getId();
	}
	 ...
	 */
	public static BodyCoveringType getMaterialBodyCoveringType(BodyMaterial material, BodyCoveringCategory category) {
		return table.of(material.toString()+"_"+category.toString());
	}

	SpecialTable table = new SpecialTable();

	final class SpecialTable extends Table<BodyCoveringType> {

		private final ArrayList<BodyCoveringType> allMakeupTypes = new ArrayList<>();

		private final ArrayList<BodyCoveringType> allSlimeTypes = new ArrayList<>();

		private SpecialTable() {
			super(BodyCoveringType::sanitize);
			forEachMod("/race","coveringTypes",null,(f,n,a)->{
				var k = n.replaceAll("coveringTypes_","");
			...
			for(BodyCoveringCategory cat : BodyCoveringCategory.values()) {
				Map<CoveringPattern,Integer> patterns;
				switch(cat) {
				case ANTENNAE:
				case ANTLER:
				case HORN:
				...
				case PENIS:
				case TONGUE:
					patterns = CoveringPattern.allSlimeCoveringPatterns;
					break;
				case EYE_IRIS:
				case EYE_PUPIL:
				case EYE_SCLERA:
					patterns = Map.of(CoveringPattern.EYE_IRISES_HETEROCHROMATIC,1);
					break;
				case ANUS:
				case MOUTH:
				case NIPPLE:
				...
				var bct = new AbstractBodyCoveringType(cat,BodyCoveringTemplateFactory.createSlime(pattern(cat),patterns)) {};
				String id = String.format("%s_%s",BodyMaterial.SLIME,cat);
				add(id,bct);
			...
			for(BodyMaterial mat : BodyMaterial.values()) {
				if(mat==BodyMaterial.FLESH || mat==BodyMaterial.SLIME)
					continue;
				String name;
				CoveringModifier modifier;
				List<Colour> naturalColours;
				switch(mat) {
				case AIR:
					name = "vapours";
					modifier = CoveringModifier.SWIRLING;
					naturalColours = List.of(PresetColour.COVERING_BLUE_LIGHT,PresetColour.COVERING_GREY,PresetColour.COVERING_WHITE);
					break;
				case ARCANE:
					name = "energy";
					modifier = CoveringModifier.SWIRLING;
					naturalColours = List.of(PresetColour.COVERING_PINK,PresetColour.COVERING_PINK_DARK,PresetColour.COVERING_PINK_LIGHT);
					break;
				case FIRE:
					name = "flames";
					modifier = CoveringModifier.BLAZING;
					naturalColours = List.of(PresetColour.COVERING_RED,PresetColour.COVERING_ORANGE,PresetColour.COVERING_YELLOW,PresetColour.COVERING_BLUE_LIGHT);
					break;
				case ICE:
					name = "ice";
					modifier = CoveringModifier.GLITTERING;
					naturalColours = List.of(PresetColour.COVERING_BLUE_LIGHT,PresetColour.COVERING_WHITE);
					break;
				case RUBBER:
					name = "rubber";
					modifier = CoveringModifier.GLOSSY;
					naturalColours = List.of(PresetColour.COVERING_BLACK);
					break;
				case STONE:
					name = "stone";
					modifier = CoveringModifier.MATTE;
					naturalColours = List.of(PresetColour.COVERING_GREY,PresetColour.COVERING_BLACK);
					break;
				case WATER:
					name = "water";
					modifier = CoveringModifier.SHIMMERING;
					naturalColours = List.of(PresetColour.COVERING_BLUE,PresetColour.COVERING_BLUE_LIGHT,PresetColour.COVERING_BLUE_DARK);
					break;
				default:
					name = "";
					modifier = CoveringModifier.SMOOTH;
					naturalColours = List.of();
				}
				for(BodyCoveringCategory cat : BodyCoveringCategory.values()) {
					if(!cat.isInfluencedByMaterialType())
						continue;
					var bct = new AbstractBodyCoveringType(cat,BodyCoveringTemplateFactory.createElemental(name,modifier,pattern(cat),naturalColours)) {};
					String id = String.format("%s_%s",mat,cat);
					add(id,bct);
				}
			}
		}
	...
	@Deprecated
	public static List<BodyCoveringType> getAllBodyCoveringTypes() {
		return table.list();
	}
	
	public static List<BodyCoveringType> getAllMakeupTypes() {
		return table.allMakeupTypes;
	}
	
	public static List<BodyCoveringType> getAllSlimeTypes() {
		return table.allSlimeTypes;
	}

	private static CoveringPattern pattern(BodyCoveringCategory category) {
		switch(category) {
		case EYE_IRIS:
			return CoveringPattern.EYE_IRISES;
		case EYE_PUPIL:
			return CoveringPattern.EYE_PUPILS;
		case EYE_SCLERA:
			return CoveringPattern.EYE_SCLERA;
		case ANUS:
			return CoveringPattern.ORIFICE_ANUS;
		case MOUTH:
			return CoveringPattern.ORIFICE_MOUTH;
		case NIPPLE:
			return CoveringPattern.ORIFICE_NIPPLE;
		case NIPPLE_CROTCH:
			return CoveringPattern.ORIFICE_NIPPLE_CROTCH;
		case VAGINA:
			return CoveringPattern.ORIFICE_VAGINA;
		case SPINNERET:
			return CoveringPattern.ORIFICE_SPINNERET;
		default:
			return CoveringPattern.NONE;
		}
	}
}