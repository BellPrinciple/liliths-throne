...
//	};
	...
	FeralAttributes getFeralAttributes();

	default boolean isFeralConfigurationAvailable() {
	 ...
	 */
	boolean isAquatic(GameCharacter character);
	...
	default boolean isFurryPreferencesEnabled() {
		return !hasFlag(SubspeciesFlag.DISABLE_FURRY_PREFERENCE);
	}
	...
	public static AbstractSubspecies DOG_MORPH_BORDER_COLLIE = new AbstractSubspecies(false,
			...
			Util.newHashMapOfValues(
					...
					new Value<>(Attribute.SPELL_COST_MODIFIER, 5f),
					new Value<>(Attribute.HEALTH_MAXIMUM, 5f),
					new Value<>(Race.DamageAttribute.of(Race.getRaceFromId("innoxia_sheep")), 25f)),
			null,
			"Canine Culture",
			"Canine Cultures",
			"DOG_MORPH_BASIC",
		...
		public int getSubspeciesWeighting(Body body, Race race) {
			if(race==Race.DOG_MORPH) {
				var canineFur = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_FUR):BodyCoveringType.CANINE_FUR;
	...
	public static AbstractSubspecies FOX_MORPH_ARCTIC = new AbstractSubspecies(false,
		...
		public int getSubspeciesWeighting(Body body, Race race) {
			if(race==Race.FOX_MORPH) {
				var foxFur = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_FUR):BodyCoveringType.FOX_FUR;
	...
	public static AbstractSubspecies CAT_MORPH_LYNX = new AbstractSubspecies(false,
		...
		public int getSubspeciesWeighting(Body body, Race race) {
			if(race==Race.CAT_MORPH) {
				...
				var felineFur = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_FUR):BodyCoveringType.FELINE_FUR;
...
//	};
	...
	public static AbstractSubspecies HORSE_MORPH_ZEBRA = new AbstractSubspecies(false,
		...
		public int getSubspeciesWeighting(Body body, Race race) {
			if(race==Race.HORSE_MORPH) {
				var horseHair = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_HAIR):BodyCoveringType.HORSE_HAIR;
...
//					return 110; // Less than zebra
	...
	public static AbstractSubspecies HARPY_RAVEN = new AbstractSubspecies(false,
		...
		public int getSubspeciesWeighting(Body body, Race race) {
			if(race==Race.HARPY) {
				var feathers = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_FEATHER):BodyCoveringType.FEATHERS;
	...
	public static AbstractSubspecies HARPY_SWAN = new AbstractSubspecies(false,
		...
		public int getSubspeciesWeighting(Body body, Race race) {
			if(race==Race.HARPY) {
				var feathers = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_FEATHER):BodyCoveringType.FEATHERS;
				var legSkin = body.getBodyMaterial()==BodyMaterial.SLIME?BodyCoveringType.getMaterialBodyCoveringType(BodyMaterial.SLIME, BodyCoveringCategory.MAIN_SKIN):BodyCoveringType.HARPY_SKIN;
	...
	static Map<Subspecies,SubspeciesSpawnRarity> getDominionStormImmuneSpecies(boolean onlyCoreRaceSpecies, Subspecies... subspeciesToExclude) {
		...
		for(var s : table.list()) {
			if(s.getRace()!=Race.DEMON
				...
				continue;
			var t = s.getWorldLocations().get(WorldType.DOMINION);
			if(null==t)
				continue;
			var m = s.getStatusEffectAttributeModifiers(null).get(Attribute.MAJOR_ARCANE);
